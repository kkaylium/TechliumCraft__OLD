...
import net.kkaylium.mods.TechliumCraft.init.gen.tree.DarkTreeGen;
...
public class BlockDarkSapling extends BlockSapling {
    ...
    public BlockDarkSapling(){
        ...
        this.setBlockName(TCNames.darkSaplingName);
        float f = 0.4F;
        setBlockBounds(0.5F - f, 0.0F, 0.5F - f, 0.5F + f, f * 2.0F, 0.5F + f);
        this.setHardness(0.0F);
        this.setStepSound(Block.soundTypeGrass);
        this.setBlockTextureName(ModInfo.MOD_ID + ":darkSapling");
    }

    @Override
    @SideOnly(Side.CLIENT)
    public void registerBlockIcons(IIconRegister iconregister) {
        texture = iconregister.registerIcon(ModInfo.MOD_ID + ":" + "darkSapling");
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9) {
        if (!world.isRemote) {
            if (player.inventory.getCurrentItem().getItem() == TCInits.darkCrystal_WHITE) {
                player.inventory.consumeInventoryItem(TCInits.darkCrystal_WHITE);
                return world.isRemote || new DarkTreeGen().generate(world, world.rand, x, y, z);
            }
            return true;
        } else {
            return false;

...
public class WorldGenGlowTree extends WorldGenAbstractTree{

	private boolean field_150531_a;
	
	public WorldGenGlowTree(boolean par1, boolean p_i45449_2_){
		super(par1);
		...
		if (par4 >= 1 && par4 + l + 1 <= 256) {
			...
			if (!flag) {
				...
				if (isSoil && par4 < 256 - l - 1) {
					...
					for (k2 = par4 - 3 + l; k2 <= par4 + l; ++k2) {
						...
						for (int l2 = par3 - k1; l2 <= par3 + k1; ++l2) {
							...
							for (int i2 = par5 - k1; i2 <= par5 + k1; ++i2) {
								...
								if (Math.abs(l1) != k1 || Math.abs(j2) != k1
										|| par2Random.nextInt(2) != 0
										&& j1 != 0) {
									Block block1 = par1World.getBlock(l2, k2,
											i2);

									if (block1.isAir(par1World, l2, k2, i2)
											|| block1.isLeaves(par1World, l2,
													k2, i2)) {
					...
					for (k2 = 0; k2 < l; ++k2) {
						Block block3 = par1World
								.getBlock(par3, par4 + k2, par5);

						if (block3.isAir(par1World, par3, par4 + k2, par5)
								|| block3.isLeaves(par1World, par3, par4 + k2,
										par5)) {
							this.setBlockAndNotifyAdequately(par1World, par3,
									par4 + k2, par5, Blocks.log, 2);
						}
					}
					return true;
				} else {

package kkaylium.mods.TechliumCraft.glowglass.info;

/**
 * Created by kkaylium on 10/26/14.
 */
public class GGInfo {

    public static String glow_block_name = "glowblock";
    public static String glow_block_igname = "Glow Block";
    public static String glow_glass_name = "glowglass";
    public static String glow_glass_igname = "Glow Glass";
    public static String fancy_glass_name = "fancyglass";
    public static String super_fancy_glass_name = "superfancyglass";
    public static String glow_fence_name = "glowfence";
    public static String glow_fence_igname = "Glow Fence";
    public static String glow_gate_name = "glowgate";
    public static String glow_gate_igname = "Glow Gate";
    public static String glow_ore_name = "glowore";
    public static String glow_ore_igname = "Glow Ore";
    public static String glow_leaves_name = "glowleaves";
    public static String glow_leaves_igname = "Glow Leaves";
    public static String glow_log_name = "glowlog";
    public static String glow_log_ignames = "Glow Log";
    public static String glow_planks_name = "glowplanks";
    public static String glow_planks_igname = "Glow Planks";
    public static String glow_stick_name = "glowstick";
    public static String glow_stick_igname = "Glow Stack";
    public static String glow_sapling_name = "glowsapling";
    public static String getGlow_sapling_igname = "Glow Sapling";

    public static String glow_crystal_name = "glowcrystal";
    public static String glow_crystal_igname = "Glow Crystal";

    public static String glow_tool_name = "glowtool";
    public static String glow_tool_igname = "Glow Tool";
    public static String rainbow_tool_name = "rainbowtool";
    public static String rainbow_tool_igname = "Rainbow Tool";

    public static String TE_GLOW_NAME = "tileentityglow";
}
